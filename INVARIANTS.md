...
| stageMerge (snapshot) → gate build → abortMerge (ignore listing + residue sweep) (#92) | `git.service.ts` under `landing.service.ts` | RETRIED / SELF-HEALED — the snapshot persists beside MERGE_HEAD, so a death while the merge is still staged re-enters through startup's `selfHealStagedMerge`, whose abort re-captures the staged ignore listing and sweeps; a death between the abort and the sweep leaves the snapshot the startup sweep finishes | `git.service.test.ts` "sweepStageResidue converges from a persisted snapshot after a death mid-sweep (#92)", `daemon.run.test.ts` "self-heal finishes a residue sweep a death interrupted, and logs it (#92)" |
...
| bootout-drain wait → write resolved → install → record → start | `supervisor.run.ts` `runUp` apply, starting a `stopping` job (#93) | RETRIED — launchd's bootout is asynchronous, so a downed job stays listed until its process exits and bootstrapping over it fails; the wait mutates nothing, so a timeout or death during the drain leaves the teardown untouched and the next `score up` starts the job once the drain ends; a death after the resolved write, before install, leaves an unregistered definition the next `up` plans as a start; a death after install leaves a registered job the supervisor launches itself (launchd KeepAlive implies RunAtLoad) whose stale record the next `up` heals as a definition-drift restart; a death after the record write leaves that self-launched job correctly recorded, so the next `up` reports it unchanged | `supervisor.run.test.ts` "down then keyed up during the bootout drain waits it out and starts (#93)", "a drain outlasting the wait fails loudly with nothing mutated; the next up converges (RETRIED)", "start after a drain that fails at install: the next up re-installs and starts (RETRIED)", "a drifted rendered definition restarts a job even when the hash is unchanged" (the post-install death state), and "second up with no config change performs zero launchctl mutations" (the post-record death state) |
| read definition → stop → install → record → start | `supervisor.run.ts` `runRestart` | RETRIED / SELF-HEALED — read-before-stop refuses to boot out a job with no restorable definition; a death after stop leaves a definition-only job the next `score up` re-installs and starts; a death after install leaves a registered job the supervisor launches itself (launchd KeepAlive implies RunAtLoad, systemd installs with `enable --now`) and a retried restart converges; concurrent mutations on one project (`up`, `restart`, and `down`) are serialized by a per-project lockfile — the contended command fails without mutating, and a lock whose holder is dead is broken, so a death while holding it costs one retried command | `supervisor.run.test.ts` "restart with no saved definition …", "restart step failure at stop …", "restart death after stop …", "restart death after install …", "restart racing a config-changing up …", "a live holder's lock blocks …", "a stale lock (dead holder or garbage) is broken …" |
